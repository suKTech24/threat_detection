...
def process_sql_file(file_path):
    ...
    content = insert_pattern.sub(r'\1\2)', content)
    
    # Regex to capture fields inside SELECT ...
    select_pattern = re.compile(r'(SELECT\s+.*?)(fieldD).*?FROM', re.DOTALL)
    content = select_pattern.sub(r'\1\2 FROM', content)
